...
func TestIntegrationCargoViews(t *testing.T) {
	...
	// Initialize cargo views (load from SQL file)
...
// initializeCargoViews creates the cargo views (simplified for testing)
func initializeCargoViews(db *sql.DB) error {
	viewSQL := `
		-- v_item_volumes: Item volume and pricing information
		CREATE VIEW v_item_volumes AS
		SELECT 
			t._key as type_id,
			json_extract(t.name, '$.en') as item_name,
			COALESCE(t.volume, 0) as volume,
			COALESCE(t.capacity, 0) as capacity,
			COALESCE(t.packagedVolume, 0) as packagedVolume,
			COALESCE(t.basePrice, 0) as basePrice,
			g.categoryID as category_id,
			json_extract(c.name, '$.en') as category_name,
			t.marketGroupID,
			CASE 
				WHEN t.volume > 0 THEN t.basePrice / t.volume
				ELSE 0
			END as isk_per_m3
		FROM types t
		...
		-- v_ship_cargo_capacities: Ship cargo hold information
		CREATE VIEW v_ship_cargo_capacities AS
		SELECT 
			t._key as ship_type_id,
			json_extract(t.name, '$.en') as ship_name,
			COALESCE(t.capacity, 0) as base_cargo_capacity
		FROM types t
		LEFT JOIN groups g ON t.groupID = g._key
