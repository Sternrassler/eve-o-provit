...
1. **Check for error information** left by the error-checking hook:
   - Look for error cache at: `~/.ai-cache/tsc-errors/[session_id]/last-errors.txt`
   - Check affected repos at: `~/.ai-cache/tsc-errors/[session_id]/affected-repos.txt`
   - Get TSC commands at: `~/.ai-cache/tsc-errors/[session_id]/tsc-commands.txt`
...
cat ~/.ai-cache/tsc-errors/*/last-errors.txt

# 2. Check which TSC commands to use
cat ~/.ai-cache/tsc-errors/*/tsc-commands.txt
...
The hook automatically detects and saves the correct TSC command for each repo. Always check `~/.ai-cache/tsc-errors/*/tsc-commands.txt` to see which command to use for verification.
